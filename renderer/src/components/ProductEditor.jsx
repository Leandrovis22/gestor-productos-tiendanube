...
const VariantsForm = ({
  ...
  typeName, setTypeName, typeValues, setTypeValues,
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
                ...
                <div className="max-h-32 overflow-y-auto mb-2 bg-gray-800 rounded p-2">
                  {predefinedColors.map(color => (
                    <label key={color} className="flex items-center gap-2 py-1 cursor-pointer">
                      <input type="checkbox" checked={selectedColors.includes(color)} onChange={() => onToggleColor(color)} className="rounded" />
                ...
                <div className="grid grid-cols-2 gap-1 mb-2">
                  {predefinedSizes.map(size => (
                    <label key={size} className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={selectedSizes.includes(size)} onChange={() => onToggleSize(size)} className="rounded" />
          ...
          <div className="border border-gray-600 rounded p-3">
            <div className="flex items-center gap-2 mb-3">
              ...
              <input type="text" value={typeName} onChange={(e) => setTypeName(e.target.value)} className="flex-1 bg-gray-800 border border-gray-600 rounded px-2 py-1 text-sm" />
            </div>
            {useType && (
              <div>
                <label className="block text-xs text-gray-400 mb-1">Valores:</label>
                <textarea value={typeValues} onChange={(e) => setTypeValues(e.target.value)} className="w-full h-16 bg-gray-800 border border-gray-600 rounded px-2 py-1 text-sm" />
    ...
    switch (activeTab) {
      ...
      case 'variantes':
        return <VariantsForm 
          ...
          selectedSizes={selectedSizes} onToggleSize={onToggleSize} onSelectAllSizes={onSelectAllSizes} onClearAllSizes={onClearAllSizes} predefinedSizes={predefinedSizes}
